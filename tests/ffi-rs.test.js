...
const skip = !fs.existsSync(fixturePath) || !hasFfiRs();
const scripts = ['add-i32', 'add-f64', 'getpid', 'many-args', 'pointer-bigint', 'sum-buffer'];

for (const script of scripts) {
  test(`ffi-rs/${script} produces a valid JSON sample`, { skip }, () => {
    const res = spawnSync(
      process.execPath,
      [`benchmarks/ffi-rs/${script}.js`],
      {
        cwd: repoRoot,
        encoding: 'utf8',
    ...
    assert.strictEqual(parsed.backend, 'ffi-rs');
